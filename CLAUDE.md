...
**Client-side only**: The main page uses `dynamic(() => ..., { ssr: false })` so all Novu hooks and Mixpanel run exclusively in the browser.

**Data flow**: `UnsubscribePage` (outer) identifies the subscriber via URL params and wraps everything in `NovuProvider`. `UnsubscribeContent` (inner) manages the unsubscribe flow state machine with three states:
...
- `NEXT_PUBLIC_NOVU_APP_ID` — Novu application identifier (has hardcoded fallback)
- `NEXT_PUBLIC_NOVU_API_KEY` — Novu API key
